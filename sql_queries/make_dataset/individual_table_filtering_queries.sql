COPY 
(SELECT t1.patientunitstayid, t4.aki_in_diagnosis
FROM
...
INNER JOIN
-- this query labels patientunitstayid as 1/0 for aki development after 24h
(SELECT d.patientunitstayid,
CASE WHEN d.concat_diag ILIKE '%acute renal failure%' THEN 1 ELSE 0 END AS aki_in_diagnosis
FROM
(SELECT patientunitstayid, string_agg(diagnosisstring, ', ') AS concat_diag
FROM diagnosis
GROUP BY patientunitstayid) d) t4
ON t3.patientunitstayid=t4.patientunitstayid)
TO '/Users/zhannahakhverdyan/Insight/eICU_AKIlert/data/intermediate/patid_aki_label.csv' (format csv);